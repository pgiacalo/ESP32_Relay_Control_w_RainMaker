...
esp_rmaker_device_t *relay1_device;
esp_rmaker_device_t *relay2_device;

/* Function to print the ready banner */
static void printReadyBanner(void)
{
    ESP_LOGI(TAG, "***************************************************************************");
    ESP_LOGI(TAG, "***                                                                     ***");
    ESP_LOGI(TAG, "***                 DUAL RELAY CONTROLLER READY!                        ***");
    ESP_LOGI(TAG, "***                                                                     ***");
    ESP_LOGI(TAG, "***  ✅ WiFi Connected     ✅ MQTT Connected     ✅ RainMaker Ready      ***");
    ESP_LOGI(TAG, "***  ✅ Relay 1: GPIO %d     ✅ Relay 2: GPIO %d     ✅ Unlimited Control ***", RELAY_1_GPIO, RELAY_2_GPIO);
    ESP_LOGI(TAG, "***                                                                     ***");
    ESP_LOGI(TAG, "***  Use ESP RainMaker app to control your relays independently!         ***");
    ESP_LOGI(TAG, "***                                                                     ***");
    ESP_LOGI(TAG, "***************************************************************************");
}
            ...
            const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    ...
    if (strcmp(esp_rmaker_param_get_name(param), ESP_RMAKER_DEF_POWER_NAME) == 0) {
        ...
        if (device == relay1_device) {
            ...
            // Update RainMaker state to reflect actual relay state
            esp_rmaker_param_update_and_report(
                esp_rmaker_device_get_param_by_name(relay2_device, ESP_RMAKER_DEF_POWER_NAME),
                esp_rmaker_bool(val.val.b));
        }
    }
    return ESP_OK;
...
void app_main()
{
    // CRITICAL: Set GPIO pins to default states immediately to prevent relay activation during startup
    ...
    app_driver_init();
    // Initial states are set in app_driver_init()
    ...
    esp_rmaker_node_t *node = esp_rmaker_node_init(&rainmaker_cfg, "ESP RainMaker Device", "Dual Relay Controller");
    if (!node) {
     ...
     */
    esp_rmaker_device_add_cb(relay1_device, write_cb, NULL);
    esp_rmaker_device_add_cb(relay2_device, write_cb, NULL);

    /* Add the standard name parameter (type: esp.param.name), which allows setting a persistent,
     * user friendly custom name from the phone apps. All devices are recommended to have this
     * parameter.
     */
    esp_rmaker_device_add_param(relay1_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Relay 1"));
    esp_rmaker_device_add_param(relay2_device, esp_rmaker_name_param_create(ESP_RMAKER_DEF_NAME_PARAM, "Relay 2"));

    /* Add the standard power parameter (type: esp.param.power), which adds a boolean param
     * with a toggle switch ui-type.
     */
    esp_rmaker_param_t *power_param1 = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, DEFAULT_RELAY_1_STATE);
    esp_rmaker_device_add_param(relay1_device, power_param1);
    
    esp_rmaker_param_t *power_param2 = esp_rmaker_power_param_create(ESP_RMAKER_DEF_POWER_NAME, DEFAULT_RELAY_2_STATE);
    esp_rmaker_device_add_param(relay2_device, power_param2);

    /* Assign the power parameter as the primary, so that it can be controlled from the
     * home screen of the phone apps.
     */
    esp_rmaker_device_assign_primary_param(relay1_device, power_param1);
    esp_rmaker_device_assign_primary_param(relay2_device, power_param2);

    /* Add this switch device to the node */
    esp_rmaker_node_add_device(node, relay1_device);
    esp_rmaker_node_add_device(node, relay2_device);
